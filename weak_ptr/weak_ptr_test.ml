...
let () =
  Array.iter (fun p ->
    ...
    | None -> print_endline "fail pre GC"; exit 1
    | Some _ -> ()
  ) ptr
...
let () =
  Array.iter (fun p ->
    match Registry.find_opt t p with
    | None -> print_endline "fail post GC"; exit 2
    | Some _ -> ()
  ) ptr
...
let () =
  ...
  | Some _ -> ()
  | None -> print_endline "effect 3 should still be around"; exit 3

let () = Gc.full_major ()

let () =
  match Registry.find_opt t (Array.get ptr 3) with
  | Some _ -> print_endline "effect 3 should be collected"; exit 4
  | None -> ()
