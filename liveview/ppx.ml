...
let component_visitor =
  ...
  let get_lident = function
    | { pexp_desc = Pexp_ident { txt = Lident str; _ }; _ } -> Ok str
    | { pexp_loc; _ } -> Error pexp_loc
  in
  object
    ...
    method! expression ctx expr =
      ...
      | {
       pexp_desc = Pexp_extension ({ txt = "a"; _ }, _payload);
       pexp_loc = loc;
       _;
      } ->
      ...
      | [%expr [%v [%e? expr]]] ->
          (* match [%v <expr>] *)
          begin match get_lident expr with
          | Ok str ->
              let () =
      ...
      | {
       pexp_desc = Pexp_extension ({ txt = "v"; _ }, _payload);
       pexp_loc = loc;
       _;
      } ->
          Ast_builder.pexp_extension ~loc
